...
/// <summary>
/// SRP: outbox satırlarını broker'a taşır (poll → publish → mark). MigrateAndSeed'in
/// scope-per-iteration desenini örnek alır, üstüne BackgroundService poll döngüsü koyar.
/// At-least-once: publish sonrası mark'tan önce crash olursa mesaj tekrar yayınlanabilir —
/// tüketici idempotent olmalı (W10 notu). FIFO OccurredOnUtc; OrderPaid'ler bağımsız,
/// katı sıra kritik değil.
/// </summary>
    ...
    ILogger<OutboxDispatcher> logger) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                // Döngü ASLA ölmemeli: RabbitMQ kopması / geçici hata bir sonraki tur'da
                // düzelir; bekleyen satırlar tabloda durur (outbox'ın dayanıklılığı).
                logger.LogError(ex, "Outbox dispatch turu başarısız; bir sonraki poll'de yeniden denenecek");
            }
    ...
    /// <summary>
    /// Batch işleme çekirdeği — bağımlılıkları PARAMETRE olarak alır (DIP), böylece test
    /// scope/DI kurmadan doğrudan çağırabilir. ExecuteAsync bunu scope içinden besler.
    /// </summary>
        ...
        foreach (var message in batch)
        {
            ...
            catch (Exception ex)
            {
                ...
                logger.LogWarning(ex,
                    "Outbox mesajı publish edilemedi: {MessageId} (deneme {RetryCount}/{Max})",
                    message.Id, message.RetryCount, MaxRetries);
            }
        }

        // Mark + retry sayaçları tek yazımda persist edilir.
